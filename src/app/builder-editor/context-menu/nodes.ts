...
export class NewModulesNode extends ContextTreeNode {
    ...
    execute(state: ContextMenuState, { event }: { event: MouseEvent }) {
        const worldCoordinates = state.drawingArea.invert(
            event.clientX,
            event.clientY,
        )
        ImportModulesView.popupModal(
            ...
            (nodes: Array<AssetsExplorerView.ModuleItemNode>) => {
                ...
                const fluxPacks = nodes.flatMap((node) =>
                    node.fluxPacks.map((fluxPack) => fluxPack),
                )
                state.appState
                    .addLibraries$(libraries, fluxPacks)
                    .subscribe(() => {
                        nodes.forEach((node) =>
                            state.appState.addModule(
                                node.factory,
                                worldCoordinates,
                            ),
                        )
...
export class DocumentationNode extends ContextTreeNode {
    ...
    constructor(appStore: AppStore) {
        super({
            id: 'documentation',
            children: DocumentationNode.createChildren(appStore),
            name: 'documentation',
            faIcon: 'fas fa-book',
        })
    }
