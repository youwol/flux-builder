...
import {
    ...
    ChildrenResp,
    GroupsResp,
} from '../../clients/assets-browser.client'
...
export namespace AssetsExplorerView {
    ...
    export class State extends ImmutableTree.State<ExplorerTreeNode> {
        ...
        public readonly selection$: Subject<{
            node: ModuleItemNode
            selected: boolean
        }>
        static readonly favorites$ = new BehaviorSubject<Array<Favorite>>(
            getStoredFavorites(),
        )
        ...
        constructor({
            ...
            super({
                rootNode: new RootNode({
                    favorites: State.favorites$.getValue(),
                }),
            })
            this.appStore = appStore
            this.selectionBuffer$ = selectionBuffer$ ? selectionBuffer$ : of([])

            State.favorites$.subscribe((favorites) => {
                localStorage.setItem(
                    'flux-builder#favorites',
        ...
        toggleFavorite(node: ExplorerTreeNode) {
            ...
            const originalId =
                node instanceof FavoriteNode ? node.favorite.id : node.id
            if (favorites.find((f) => f.id == originalId)) {
                favorites = favorites.filter(
                    (favorite) => favorite.id != originalId,
                )
                State.favorites$.next(favorites)
                this.removeNode(
                    node instanceof FavoriteNode
                        ? node.id
                        : 'favorite_' + originalId,
                )
                return
            }
    ...
    function headerView(state: State, node: ExplorerTreeNode) {
        if (!(node instanceof ModuleItemNode)) {
            const favoriteClassBase =
                'fas fa-star fa-xs fv-hover-opacity fv-pointer '
            return {
                ...
                children: [
                    ...
                    node instanceof FavoriteNode
                        ? {
                              tag: 'i',
                              class: 'fas fa-star fa-xs px-1 fv-text-focus fv-hover-opacity',
                              onclick: (ev) => {
                                  ev.stopPropagation()
                    ...
                    node instanceof AssetFolderNode
                        ? {
                              tag: 'i',
                              class: attr$(State.favorites$, (favorites) =>
                                  favorites.find((f) => f.id == node.id)
                        ...
                        : {},
                    child$(node.status$, (statusList) =>
                        statusList.find(
                            (status) => status.type == 'request-pending',
                        )
        ...
        if (node instanceof ModuleItemNode) {
            return {
                ...
                onclick: () =>
                    state.selection$.next({ node: node, selected: true }),
                children: [
                    {
                        tag: 'i',
                        class: attr$(
                            ...
                            {
                                wrapper: (d) =>
                                    'fas fa-cloud-download-alt fv-text-primary fv-hover-opacity ' +
                                    d,
                            },
                        ),
                    },
    ...
    type TChildren =
        ...
        | Observable<Array<ExplorerTreeNode>>
    ...
    class AssetItemNode extends ItemNode {
        ...
        constructor({
            ...
            super({
                ...
                name,
                type: 'AssetItemNode',
                children: children ? children : getModules$(rawId),
                faIcon: 'fas fa-box',
            })
    ...
    class FavoriteNode extends ItemNode {
        ...
        constructor({ favorite }: { favorite: Favorite }) {
            super({
                ...
                type: 'FavoriteNode',
                children:
                    FavoriteNode.getChildrenFactory[favorite.type](favorite),
    ...
    function getRootChildren$() {
        ...
        return getGroupChildren$().pipe(
            map((children) => [
                ...children,
                ...favorites.map((favorite) => new FavoriteNode({ favorite })),
            ]),
        )
    }
    ...
    function getGroupChildren$(
        path = '',
    ): Observable<Array<GroupNode | DriveNode>> {
                    ...
                    return factories.map(
                        (v) =>
                            new ModuleItemNode({
                                ...
                                fluxPacks: loadingGraph.lock
                                    .flat()
                                    .filter(
                                        (library) =>
                                            library.type == 'flux-pack',
                                    ),
                            }),
                    )
                },
