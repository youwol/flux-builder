...
export namespace AutoForm {
    ...
    export class State {
        ...
        outputValues$ = new Array<
            Observable<{ path: Array<string>; value: ValueType }>
        >()
    ...
    export const viewFactory = [
        ...
        },
        {
            ...
            view: (
                ...
                const items = description.metadata.enum.map(
                    (text) => new Select.ItemData(text, text),
                )
        ...
        },
        {
            test: (value: ValueDescription) => {
                return (
                    value.type == 'Number' &&
                    value.metadata.min != undefined &&
                    value.metadata.max != undefined
                )
            },
            view: (
                ...
                const state = new Slider.State({
                    min: description.metadata.min,
                    max: description.metadata.max,
                    value: value$,
                    count: 1000,
                })
                return {
                    ...
                    children: [
                        {
                            tag: 'input',
                            class: attr$(hovered$, (isHovered) =>
                                isHovered ? 'w-100' : 'w-25',
                            ),
                            type: 'number',
                            value: attr$(value$, (v) => v),
                            onchange: (event) =>
                                value$.next(Number(event.target.value)),
    ...
    function createAttributesGrid(
        ...
        const order = Object.keys(
            basePath.reduce((acc, e) => acc[e], configurationBase),
        )

        items = items.sort(
            (lhs, rhs) => order.indexOf(lhs[0]) - order.indexOf(rhs[0]),
        )
        if (items.length == 0) {
            return {}
        }
    ...
    function isLeaf(v: ValueDescription) {
        return (
            v.type && ['Number', 'String', 'Boolean', 'Object'].includes(v.type)
        )
    }
    ...
    function leafView(
        ...
        const factory = state.elementsViewFactory.find((element) =>
            element.test(valueMeta),
        )
        if (!factory) {
            return {
                id: 'unkwnown-element-' + valueMeta.name + '-' + valueMeta.type,
            }
        }
        const value$ = new BehaviorSubject(value0)
        const view = factory.view(value$, valueMeta)
        const obs = (value$ as Observable<any>).pipe(
    ...
    function groupItems(
        ...
        const r = Object.entries(children)
            .map(([prefix, { values, basePath }]: any) => [
                prefix,
                groupItems(values, configurationBase, basePath, state),
            ])
            .reduce((acc, [k, v]) => Object.assign({}, acc, { [k]: v }), {})

        const itemsView = createAttributesGrid(
            currentItems,
            basePath,
            configurationBase,
            state,
