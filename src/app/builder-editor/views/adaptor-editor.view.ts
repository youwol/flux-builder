...
export namespace AdaptorEditoView {
    ...
    function parseError(stack: string): ExecutionError {
        try {
            ...
            lines = lines.filter(
                (line) => line.includes('eval') && line.split(',').length == 2,
            )
            if (lines.length == 0) {
    ...
    export class State {
        ...
        public readonly adaptedInput$: Observable<{
            data
            configuration
            context
        }>
        ...
        constructor({
            ...
            this.rawInput$ = this.mdleStart.getOutputSlot(
                connection.start.slotId,
            ).observable$

            this.contract = this.mdleEnd.getInputSlot(
                this.connection.end.slotId,
            ).contract
                ...
                map(([content, data]) => {
                    try {
                        const result = new Function(content)()(
                            data,
                            this.mdleEnd.helpers,
                        )
                        result.configuration = _.merge(
                            {},
                            this.mdleEnd.getPersistentData(),
    ...
    export class View implements VirtualDOM {
        static defaultOptions = {
            containerClass:
                'fv-bg-background p-3 fv-text-primary rounded d-flex h-100 w-100',
            containerStyle: {},
            untilFirst: {
                ...
                children: [
                    {
                        tag: 'p',
                        innerText:
                            'No data available: the module has not played any scenario yet. Having data available may help to write your code.',
                    },
                    {
                        tag: 'p',
                        innerText:
                            'Getting some data is usually as easy as connecting the input(s) of your module.',
                    },
                ],
            },
        }
        ...
        dataColumnView(): VirtualDOM {
            return {
                ...
                style: { 'font-family': 'monospace' },
                children: [this.rawInputView(), this.adaptedInputView()],
            }
        }

        statusView(): VirtualDOM {
            const info =
                "Presented here is a blind test of the input that is reaching the module's implementation " +
            ...
            return {
                ...
                children: [
                    infoView(info),
                    child$(this.state.adaptedInput$, (input) => {
                        if (
                            input instanceof ExecutionError /*|| !input.data*/
                        ) {
                        ...
                        return new InputStatusView.View({
                            ...
                            options: {
                                containerClass:
                                    'flex-grow-1 w-100 overflow-auto',
        ...
        rawInputView(): VirtualDOM {
            const info =
                'Presented here is the latest input that have reached the adaptor, before any transformation. ' +
        ...
        adaptedInputView(): VirtualDOM {
            const info =
                "Presented here is the input that is actually reaching the module's implementation. " +
        ...
        dataTreeView(
            input$: Observable<{ data; configuration; context }>,
            rootNodeName,
        ) {
            let expandedNodes = [rootNodeName + '_0']

            return {
                class: 'cm-s-blackboard overflow-auto flex-grow-1',
                            ...
                            const treeView = new DataTreeView.View({
                                ...
                                connectedCallback: (elem) => {
                                    elem.subscriptions.push(
                                        treeState.expandedNodes$.subscribe(
                                            (nodes) => (expandedNodes = nodes),
                                        ),
                                    )
                                },
                            } as any)
