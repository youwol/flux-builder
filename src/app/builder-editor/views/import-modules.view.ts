...
export namespace ImportModulesView {
    class State {
        ...
        constructor({ ok$ }: { ok$: Subject<MouseEvent> }) {
            this.ok$ = ok$
    ...
    export class View implements VirtualDOM {
        ...
        constructor({
            ...
            this.children = [this.bufferColumnView(), this.explorerView()]

            this.connectedCallback = (elem) => {
                elem.subscriptions.push(
                    this.state.explorerState.selection$
                        .pipe(
                            scan(
                                (acc, { node, selected }) => [
                                    ...acc.filter((e) => e != node),
        ...
        bufferColumnView() {
            ...
            return {
                class: 'px-2 d-flex flex-column w-25',
                style: { width: '200px' },
                children: [
                    {
                    ...
                    child$(this.state.buffer$, (nodes) => {
                                ...
                                children: nodes.map((node) => ({
                                    ...
                                    children: [
                                        { innerText: node.name },
                                        {
                                            ...
                                            onclick: () => {
                                                this.state.explorerState.selection$.next(
                                                    { node, selected: false },
                                                )
                                            },
                                        },
                        ...
                        return {
                            ...
                            class: 'py-2',
                            innerText:
                                'Pick one or more module(s) using the tabs on the right side to add them in your worksheet',
                            style: {
                                'font-style': 'italic',
                                'text-align': 'justify',
                            },
