    ...
    export class State {
        ...
        constructor({ appStore }: { appStore: AppStore }) {
            ...
            if (location.hostname == 'localhost') {
                selectItems.push(
                    new Select.ItemData('localhost-url', 'Localhost URL'),
                )
            }

            this.selectState = new Select.State(selectItems, 'youwol-url')

            this.sourceURI$ = combineLatest([
                appStore.projectURI$(),
                this.selectState.selectionId$,
            ])
        }
        ...
        constructor({ state, ...rest }: { state: State }) {
            ...
            this.connectedCallback = (elem) => {
                elem.subscriptions.push(
                    copyLinkBttn.state.click$
                        .pipe(
                            mergeMap(() => state.sourceURI$),
                            mergeMap(([uri, mode]) =>
                                from(
                                    navigator.clipboard.writeText(
                                        state.toUrl(uri, mode),
                                    ),
                                ),
                            ),
                        )
            ...
            this.children = [
                {
                    innerText:
                        'The following url can be used to share your application:',
                },
                    ...
                    children: [
                        {
                            ...
                            innerText: attr$(state.sourceURI$, ([uri, mode]) =>
                                state.toUrl(uri, mode),
                            ),
                        },
                        copyLinkBttn,
                ...
                },
                {
                    ...
                    children: [
                        ...
                        {
                            innerText:
                                "This feature is a work in progress, it is expected to work with 'relatively' small application for now. " +
