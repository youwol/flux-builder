...
function drawPlugin(
    ...
    if (containerGroup.getBBox) {
        dyModule =
            containerGroup.getBBox().height -
            containerGroup.querySelector('.content').getBBox().height / 2
    }
        ...
        this.debugSingleton.debugOn &&
            this.debugSingleton.logWorkflowView({
                ...
                object: {
                    ...
                    plugins: this.appStore.project.workflow.plugins,
                    plugInsToPlot,
                },
            })

        this.groups = plugInsToPlot
            .map(([plugin, parentSvgGroup]) => [
                plugin.moduleId,
                drawPlugin(plugin, parentSvgGroup, this.appStore),
            ])
            .reduce((acc, e) => {
                acc[e[0]] = e[1]
                return acc
