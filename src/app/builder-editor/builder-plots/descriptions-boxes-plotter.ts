...
function drawBoxes(
    ...
    const plotData = descriptionsBoxes.map((box) => {
        ...
        const selected =
            appStore.descriptionBoxSelected &&
            appStore.descriptionBoxSelected.descriptionBoxId ==
        ...
        return {
            ...
            data: { descriptionBox: box, boundingBox: rect },
        }
    })
    const plotter = new CrossPlot({
        plotId: 'descriptionsBoxPlotter',
        plotClasses: [],
        drawingArea: drawingArea,
        entities: plotData,
    })
    ...
    plotter.defaultElementDisplay = (d) => {
       ...
       <rect height="${height}" width="${width}" 
        ...
        <path d="M${-width / 2},${headerHeight - padding - height / 2} v${-(
            headerHeight - 10
        )} q0,-10 10,-10 h${width - 20} q10,0 10,10  v${headerHeight - 10} z" 
                class="description-box mdle-color-fill header " />
        <path d="M${-width / 2},${headerHeight - padding - height / 2} v${-(
            headerHeight - 10
        )} q0,-10 10,-10 h${width - 20} q10,0 10,10  v${headerHeight - 10} " 
                class="description-box header outline" />

        <text class="description-box title" x="${-width / 2 + 10}" y="${
            -height / 2 - 5
        }" >${d.data.descriptionBox.title}</text>
        `
        return g
    }
    const drawnElements = plotter.draw(plotData)
    const format = (d) =>
        d.filter((g) => g).reduce((acc, e) => acc.concat(e), [])

    return drawnElements.entered._groups
...
export class DescriptionsBoxesPlotter {
    ...
    constructor(
        ...
        this.appObservables.descriptionsBoxesUpdated$.subscribe(
            (descriptionsBoxes) => {
                const svgElements = drawBoxes(
                    descriptionsBoxes,
                    this.drawingArea,
                    this.appStore,
                )
                this.connectUserInteractions(svgElements)
                this.plottersObservables.descriptionsBoxesDrawn$.next(
                    svgElements,
                )
            },
        )
    ...
    connectUserInteractions(svgElements) {
        svgElements.forEach(
            (g: SVGElement) =>
                (g.onclick = (event: any) =>
                    this.appStore.selectDescriptionBox(g.id)),
        )
    }
}
