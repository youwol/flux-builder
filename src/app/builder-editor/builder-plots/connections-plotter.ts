...
function connectionDisplay(d) {
    ...
    const norm  = Math.pow( (d.x2-d.x1)*(d.x2-d.x1) + (d.y2-d.y1)*(d.y2-d.y1) , 0.5)
    const cos_a = (d.x1-d.x2)/ norm
    const sin_a = (d.y1-d.y2)/ norm
    const angle = ( -1 + 2*Number(d.y2<d.y1))*Math.acos( (d.x1-d.x2)/ norm ) * 180 /3.14
...
function toPlotterConnectionEntity(
    ...
    const props = appStore.project.builderRendering.connectionsView.find(cView => cView.connectionId == c.connectionId)
...
function drawWirelessSlots( drawingArea: DrawingArea, appStore: AppStore){
    const wirelesses   = drawingArea.svgCanvas.node().querySelectorAll(".wireless")
    ...
    wirelesses.forEach( g => {
        ...
        const delta = 25        
        const x1 = Number(g.getAttribute("x1"));const y1 = Number(g.getAttribute("y1"))
        const x2 = Number(g.getAttribute("x2"));const y2 = Number(g.getAttribute("y2"))  
...
function drawConnections(connections , modulesGroup, drawingArea : DrawingArea, plotObservables$, appStore : AppStore){
    ...
    const connectionsPlotData = connections.map( c =>  toPlotterConnectionEntity(c,modulesGroup,appStore ) )
...
function getSlot(
    ...
    const slotId = domPlugElement.getAttribute("slotId") || domPlugElement.getAttribute("slotid")
    ...
    if( mdle instanceof GroupModules.Module && slotId){
        ...
        const moduleId = domPlugElement.getAttribute("moduleId") || domPlugElement.getAttribute("moduleid")
...
export class ConnectionsPlotter{
        ...
        const startConnectionSubscription = (obs$,type) => {
            obs$.pipe(
                operators.filter( _ => this.drawingConnection == undefined && this.connectionCreationEnabled )
                    ...
                    const [mdl, mdlGroup] = getMdlWithGroup(d.event.target, appStore)
    ...
    plotDrawingConnection(drawingConnection, coordinates ){
        ...
        const coors= [ (coordinates[0]-1 - this.drawingArea.overallTranform.translateX)/ this.drawingArea.overallTranform.scale,
                    (coordinates[1]-1 - this.drawingArea.overallTranform.translateY)/ this.drawingArea.overallTranform.scale] 
        const selection = this.drawingArea.drawingGroup.selectAll(".drawing-connection")
        .data([{ 
