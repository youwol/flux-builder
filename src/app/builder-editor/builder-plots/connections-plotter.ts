...
function connectionDisplay(d) {
    if (!d.data.adaptor) {
        const data = {
            tag: 'g',
            attributes: { x1: d.x1, y1: d.y1, x2: d.x2, y2: d.y2 },
            children: {
                connection: {
                    tag: 'path',
                    class: 'connection-path',
                    attributes: {
                        d: `M${d.x1},${d.y1} C${0.5 * (d.x1 + d.x2)},${d.y1} ${
                            0.5 * (d.x1 + d.x2)
                        },${d.y2} ${d.x2},${d.y2}`,
                    },
                },
    ...
    const norm = Math.pow(
        (d.x2 - d.x1) * (d.x2 - d.x1) + (d.y2 - d.y1) * (d.y2 - d.y1),
        0.5,
    )
    const cos_a = (d.x1 - d.x2) / norm
    const sin_a = (d.y1 - d.y2) / norm
    const angle =
        ((-1 + 2 * Number(d.y2 < d.y1)) *
            Math.acos((d.x1 - d.x2) / norm) *
            180) /
        3.14
    const x1 = d.x1 - 50 * cos_a
    const y1 = d.y1 - 50 * sin_a
    const gAdaptor = svgAdaptorGroup
    gAdaptor.attributes = {
        transform: `translate(${-(d.x2 - d.x1) / 2 - 50 * cos_a},${
            -(d.y2 - d.y1) / 2 - 50 * sin_a
        } ) rotate(${angle + 45})  translate(-0,-36) scale(0.1)`,
    }
    const data = {
        tag: 'g',
        attributes: { x1: x1, y1: y1, x2: d.x2, y2: d.y2 },
        children: {
            connection: {
                tag: 'path',
                class: 'connection-path',
                attributes: {
                    d: `M${d.x2},${d.y2} C${0.5 * (x1 + d.x2)},${d.y2} ${
                        0.5 * (x1 + d.x2)
                    },${0.5 * (y1 + d.y2)} ${x1},${y1}`,
                    x1: x1,
                    y1: y1,
...
function retrieveSvgContainerGroup(
    ...
    const svgGroup = modulesGroup[moduleId]
    if (svgGroup) {
        return svgGroup
    }
    const container = appStore.getParentGroup(moduleId)
...
function toPlotterConnectionEntity(
    ...
    modulesGroup: GroupModules.Module,
    appStore: AppStore,
): PlotterConnectionEntity {
    const props = appStore.project.builderRendering.connectionsView.find(
        (cView) => cView.connectionId == c.connectionId,
    )
    return new PlotterConnectionEntity(
        ...
        [
            'connection',
            'mdle-color-stroke',
            toCssName(appStore.getModule(c.start.moduleId).Factory.uid),
        ].concat(props && props.wireless ? ['wireless'] : []),
...
function drawWirelessSlots(drawingArea: DrawingArea, appStore: AppStore) {
    const wirelesses = drawingArea.svgCanvas
        .node()
        .querySelectorAll('.wireless')
    const drawingGroup = drawingArea.drawingGroup.node()
    ...
    wirelesses.forEach((g) => {
        const selectFct = (event) => {
            event.stopPropagation()
            appStore.selectConnection(appStore.getConnection(g.id))
        }
        const delta = 25
        const x1 = Number(g.getAttribute('x1'))
        const y1 = Number(g.getAttribute('y1'))
        const x2 = Number(g.getAttribute('x2'))
        const y2 = Number(g.getAttribute('y2'))

        const gplugStart = document.createElementNS(
            'http://www.w3.org/2000/svg',
            'g',
        )
        const gplugEnd = document.createElementNS(
            'http://www.w3.org/2000/svg',
            'g',
        )
        gplugStart.setAttribute('transform', g.getAttribute('transform'))
        gplugEnd.setAttribute('transform', g.getAttribute('transform'))
        gplugStart.innerHTML = `<line class='' x1="${x1}" y1="${y1}" x2="${
            Number(x1) - 50
        }" y2="${y1}"></line><g transform="translate(${x1 - 50 - 2 * delta},${
            y1 - delta
        }) scale(0.1)" > ${wirelessIcon}</g>`
        gplugEnd.innerHTML = `<line class='' x1="${x2}" y1="${y2}" x2="${
            x2 + 50
        }" y2="${y2}"></line><g transform="translate(${x2 + 50},${
            y2 - delta
        }) scale(0.1)" > ${wirelessIcon}</g>`
...
function drawConnections(
    ...
    const connectionsPlotData = connections.map((c) =>
        toPlotterConnectionEntity(c, modulesGroup, appStore),
    )
    const plot = new LinkPlot({
...
function getSlot(
    ...
    const slotId =
        domPlugElement.getAttribute('slotId') ||
        domPlugElement.getAttribute('slotid')
    ...
    if (mdle instanceof GroupModules.Module && slotId) {
        ...
        const moduleId =
            domPlugElement.getAttribute('moduleId') ||
            domPlugElement.getAttribute('moduleid')
        return mdle
            .getAllChildren(workflow)
            .find((mdle) => mdle.moduleId == moduleId)
            .getSlot(slotId)
    }
    console.warn(
        "The builder plot should define the attribute 'slotId' of the slots elements",
        domPlugElement,
    )
    ...
    slot =
        plugType === 'input'
            ? mdle.inputSlots.find(
                  (slot) =>
                      domPlugElement.id ===
                      'input-slot_' + slot.slotId + '_' + slot.moduleId,
              )
            : mdle.outputSlots.find(
                  (slot) =>
                      domPlugElement.id ===
                      'output-slot_' + slot.slotId + '_' + slot.moduleId,
              )

    return slot
}

function getMdlWithGroup(
    plugSvgElement: SVGElement,
    appStore: AppStore,
): [ModuleFlux, HTMLElement] {
    let containerSlot = plugSvgElement.parentElement
    let mdl = appStore.getModuleOrPlugin(
        containerSlot.getAttribute('moduleId') ||
            containerSlot.getAttribute('id'),
    )
    if (!mdl) {
        containerSlot = containerSlot.parentElement
        mdl = appStore.getModuleOrPlugin(
            containerSlot.getAttribute('moduleId') ||
                containerSlot.getAttribute('id'),
        )
    }
...
class DrawingConnection {
    ...
    constructor(
        ...
        ;(this.xOrigin =
            Number(domModuleElement.getAttribute('x')) +
            Number(domPlugElement.getAttribute('cx'))),
            (this.yOrigin =
                Number(domModuleElement.getAttribute('y')) +
                Number(domPlugElement.getAttribute('cy')))
...
export class ConnectionsPlotter {
    ...
    constructor(
        ...
        combineLatest(
            ...
            drawConnections(
                appStore.project.workflow.connections,
                modulesGroup,
                this.drawingArea,
                plotObservables$,
                appStore,
        ...
        const startConnectionSubscription = (obs$, type) => {
            obs$.pipe(
                operators.filter(
                    (_) =>
                        this.drawingConnection == undefined &&
                        this.connectionCreationEnabled,
                ),
            ).subscribe((d: any) => {
                const [mdl, mdlGroup] = getMdlWithGroup(
                    d.event.target,
                    appStore,
                )
                this.drawingConnection = new DrawingConnection(
                    mdl,
        ...
        const endConnectionSubscription = (obs$, type) => {
            obs$.pipe(
                operators.filter(
                    (_) =>
                        this.drawingConnection != undefined &&
                        this.drawingConnection.plugType ==
                            (type == 'input' ? 'output' : 'input'),
                ),
            ).subscribe((d: any) => {
                const [mdl, _] = getMdlWithGroup(d.event.target, appStore)
                const connection =
                    type == 'input'
                        ? new Connection(
        ...
        startConnectionSubscription(
            this.plottersObservables.plugInputClicked$,
            'input',
        )
        startConnectionSubscription(
            this.plottersObservables.plugOutputClicked$,
            'output',
        )
        endConnectionSubscription(
            this.plottersObservables.plugInputClicked$,
            'input',
        )
        endConnectionSubscription(
        ...
        this.plottersObservables.mouseMoved$
            .pipe(operators.filter((_) => this.drawingConnection !== undefined))
            .subscribe((coordinates) =>
                this.plotDrawingConnection(this.drawingConnection, coordinates),
            )

        this.appObservables.connectionSelected$.subscribe((c) =>
            document
                .getElementById(getConnectionId(c))
                .classList.toggle('selected'),
        )

        this.appObservables.unselect$.subscribe(() => {
            const connection = document.getElementById('drawing-connection')
            if (connection) {
                connection.remove()
            }
            this.drawingConnection = undefined
    ...
    plotDrawingConnection(drawingConnection, coordinates) {
        ...
        const coors = [
            (coordinates[0] - 1 - this.drawingArea.overallTranform.translateX) /
                this.drawingArea.overallTranform.scale,
            (coordinates[1] - 1 - this.drawingArea.overallTranform.translateY) /
                this.drawingArea.overallTranform.scale,
        ]
        const selection = this.drawingArea.drawingGroup
            .selectAll('.drawing-connection')
            .data([
                {
    ...
    connectUserInteractions(plotObservables$) {
        const click$ = plotObservables$.pipe(
            operators.filter((d: any) => d.action === 'click'),
        )
        click$.subscribe((d: any) =>
            this.appStore.selectConnection(d.data.data),
        )
    }
}
