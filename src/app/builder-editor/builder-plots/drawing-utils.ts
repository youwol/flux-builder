...
export function convert(bbox, matrix, drawingArea) {
    const offset = document
        .getElementById(drawingArea.svgCanvas.attr('id'))
    ...
    const bbox = modulesId
        ...
        .map((e: any) => {
            ...
            return e.getBBox
                ...
                : {
                      xmin: e.getAttribute('x'),
                      xmax: e.getAttribute('x'),
                      ymin: e.getAttribute('y'),
                      ymax: e.getAttribute('y'),
                  }
        })
        .reduce(
            (acc, e) => ({
                xmin: Math.min(acc.xmin, e.xmin),
                xmax: Math.max(acc.xmax, e.xmax),
                ymin: Math.min(acc.ymin, e.ymin),
                ymax: Math.max(acc.ymax, e.ymax),
            }),
...
export function focusElement(drawingArea: DrawingArea, svgElement: SVGElement) {
    ...
    drawingArea.lookAt(
        0.5 * (boudingBox.left + boudingBox.right),
        0.5 * (boudingBox.top + boudingBox.bottom),
    )
}

function mapToFocusCoordinate(
    activeLayerUpdated$: Observable<{ fromLayerId: string; toLayerId: string }>,
    appStore: AppStore,
) {
    return activeLayerUpdated$.pipe(
        ...
        filter(
            ({ fromLayerId, toLayerId }) =>
                fromLayerId != undefined && toLayerId != undefined,
        ),
        ...
        map(({ fromLayer, toLayer }) => {
            ...
            if (
                ...
                const targetLayer = toLayer
                    .getDirectChildren(appStore.project.workflow)
                    .find(
                        (layer) =>
                            layer instanceof GroupModules.Module &&
                            (layer == fromLayer ||
                                layer
                                    .getModuleIds()
                                    .includes(fromLayer.moduleId)),
                    )
...
export function plugLayersTransition_noTransition(
    activeLayerUpdated$: Observable<{ fromLayerId: string; toLayerId: string }>,
    appStore: AppStore,
    drawingArea: DrawingArea,
) {
    mapToFocusCoordinate(activeLayerUpdated$, appStore).subscribe((coors) =>
        drawingArea.lookAt(coors[0], coors[1]),
    )
}

export function plugLayersTransition_test(
    activeLayerUpdated$: Observable<{ fromLayerId: string; toLayerId: string }>,
    appStore: AppStore,
    drawingArea: DrawingArea,
) {
    activeLayerUpdated$.subscribe(() =>
        drawingArea.selectAll('g.connection').remove(),
    )
