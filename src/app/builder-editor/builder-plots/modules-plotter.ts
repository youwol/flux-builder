...
export function createPlot(mdle, drawingArea) {
    ...
    const plotter = new CrossPlot({
        plotId: plotId,
        plotClasses: [plotId, Factory.packId],
        drawingArea,
        entities$,
    })
...
function getCenter(currentViews) {
    ...
    const xmin = coorsParentLayer.reduce(
        (acc, e) => (acc < e[0] ? acc : e[0]),
        1e10,
    )
    const xmax = coorsParentLayer.reduce(
        (acc, e) => (acc > e[0] ? acc : e[0]),
        -1e10,
    )
    const ymin = coorsParentLayer.reduce(
        (acc, e) => (acc < e[1] ? acc : e[1]),
        1e10,
    )
    const ymax = coorsParentLayer.reduce(
        (acc, e) => (acc > e[1] ? acc : e[1]),
        -1e10,
    )
...
function drawModules(
    ...
    if (
        appStore.activeGroupId != appStore.rootComponentId &&
        ...
        const moduleViewLayer =
            displayedModulesView.parentLayer.currentGroupModuleView

        projection = (x, y) => {
            const d0 = Math.pow(
                (x - moduleViewLayer.xWorld) * (x - moduleViewLayer.xWorld) +
                    (y - moduleViewLayer.yWorld) * (y - moduleViewLayer.yWorld),
                0.5,
            )
            const cos_theta0 = (x - moduleViewLayer.xWorld) / d0
            const sin_theta0 = (y - moduleViewLayer.yWorld) / d0
            const dx =
                (d0 + factors[0]) * cos_theta0 +
                (cos_theta0 / Math.abs(cos_theta0)) * 50
            const dy =
                (d0 + factors[1]) * sin_theta0 +
                (sin_theta0 / Math.abs(sin_theta0)) * 75
    ...
    const fromModuleViewInside = (view) => ({
        x: view.xWorld,
        y: view.yWorld,
        classes: ['module'].concat(
            appStore.isSelected(view.moduleId) ? ['selected'] : [],
        ),
        attributes: { moduleId: view.moduleId },
        id: view.moduleId,
        Factory: view.Factory,
        data: { module: appStore.getModule(view.moduleId), moduleView: view },
    })
    const fromModuleOutside = (view) =>
        Object.assign({}, fromModuleViewInside(view), {
            projection: projection,
        })

    const plotsData0 =
        displayedModulesView.currentLayer.modulesView.map(fromModuleViewInside)
    const plotsData1 =
        displayedModulesView.parentLayer.modulesView.map(fromModuleOutside)

    const grouped = _.groupBy(
        [...plotsData0, ...plotsData1],
        (d) => d.Factory.uid,
    )
    const modulesDrawn = {}
    ...
    const updateMdlesDrawn = (d) => {
        d.filter((g) => g).forEach((g) => {
            if (g.id.includes('group')) {
                ...
                const mIds = m.inputSlots
                    .map((s) => s.moduleId)
                    .concat(m.outputSlots.map((s) => s.moduleId))
                mIds.forEach((mid) => {
                    modulesDrawn[mid] = g
                })
            }
    ...
    return { modulesDrawn: modulesDrawn, activeSeries: activeSeriesId }
}

function drawExpandedGroup(
    layerId: string,
    drawingArea: DrawingArea,
    appStore: AppStore,
) {
    ...
    const groupMdle = appStore.project.workflow.modules.find(
        (m) =>
            m instanceof GroupModules.Module &&
            m.moduleId == activateLayer.moduleId,
    ) as GroupModules.Module

    const displayedElements = appStore.getDisplayedModulesView()
    const includedEntities = displayedElements.currentLayer.modulesView.map(
        (g) => g.moduleId,
    )
    const rect =
        includedEntities.length > 0
            ...
            : {
                  x: drawingArea.hScale(
                      displayedElements.parentLayer.currentGroupModuleView
                          .xWorld,
                  ),
                  y: drawingArea.vScale(
                      displayedElements.parentLayer.currentGroupModuleView
                          .yWorld,
                  ),
                  width: 200,
    ...
    const all = drawnElements.entered._groups
        .concat(drawnElements.updated._groups)
...
export class ModulesPlotter {
    ...
    constructor(
        ...
        this.pluginsPlotter = new PluginsPlotter(
            this.drawingArea,
        ...
        combineLatest([
            appObservables.packagesLoaded$,
            this.plottersObservables.modulesViewUpdated$,
        ]).subscribe(() => {
            const { modulesDrawn, activeSeries } = drawModules(
                this.drawingArea,
                this.appStore,
                plotObservables$,
            )
            const pluginsDrawn = this.pluginsPlotter.draw(modulesDrawn)
            const expandedGroup = drawExpandedGroup(
                this.appStore.activeGroupId,
                this.drawingArea,
                this.appStore,
            )
            this.connectUserInteractions(
                Object.assign({}, modulesDrawn, expandedGroup),
            )
            this.emptyRemovedSeries(this.previousActiveSeriesId, activeSeries)
            this.previousActiveSeriesId = activeSeries
            this.plottersObservables.modulesDrawn$.next({
                ...modulesDrawn,
                ...pluginsDrawn,
                ...expandedGroup,
            })
    ...
    highlight(modulesId: Array<string>) {
        const htmlElems = modulesId.map((mid: string) =>
    ...
    connectUserInteractions(modulesDrawn) {
        Object.entries(modulesDrawn).forEach(
            ([moduleId, g]: [string, SVGElement]) => {
                const onclick = (event: any) => {
                    if (event.target.classList.contains('slot')) {
                        event.target.classList.contains('output')
                            ? this.plottersObservables.plugOutputClicked$.next({
                                  event,
                                  group: g,
                                  moduleId,
                              })
                            : this.plottersObservables.plugInputClicked$.next({
                                  event,
                                  group: g,
                                  moduleId,
                              })
                ...
                if (
                    this.appStore.project.workflow.plugins
                        .map((m) => m.moduleId)
                        .includes(moduleId)
                ) {
                    return
    ...
    dragSelection(d3Event, update) {
        ...
        modules.forEach((m) => {
            ...
            Array.from(plugins).forEach((gPlugin) => {
                gPlugin.setAttribute(
                    'x',
                    Number(gPlugin.getAttribute('x')) + d3Event.dx,
                )
                gPlugin.setAttribute(
                    'y',
                    Number(gPlugin.getAttribute('y')) + d3Event.dy,
                )
            })
