...
export function isGroupingModule(moduleData) {
    return ['GroupModules@flux-pack-core', 'Component@flux-pack-core'].includes(
        moduleData.factoryId,
    )
}
/*
...
export function addModule(
    ...
    const groupMdle = project.workflow.modules.find(
        (mdle) => mdle.moduleId == activeLayerId,
    )
    const persistentData = new groupMdle.Factory.PersistentData({
        ...groupMdle.getPersistentData(),
        moduleIds: groupMdle
            .getPersistentData<GroupModules.PersistentData>()
            .getModuleIds()
            .concat(moduleId),
    })
    const grpConfiguration = new ModuleConfiguration({
        ...groupMdle.configuration,
        data: persistentData,
    })
    const groupModuleNew = new groupMdle.Factory.Module(
        Object.assign({}, groupMdle, { configuration: grpConfiguration }),
    )
...
function duplicate({
    ...
    if (configuration == undefined) {
        const persistentData = new Factory.PersistentData(
    ...
    return isPlugin
        ? new mdle.Factory.Module({
              parentModule: parent
                  ? parent
                  : (mdle as PluginFlux<any>).parentModule,
...
export function updateModule(
    ...
    const getChildrenRec = (mdle: ModuleFlux): Array<any> => {
        const directChildren = project.workflow.plugins.filter(
            (plugin) => plugin.parentModule.moduleId == mdle.moduleId,
        )
        const indirectChildren = directChildren
            .map((child) => getChildrenRec(child))
            .filter((d) => d.length > 0)
        ...
        return directChildren.concat(
            indirectChildren.reduce((acc, e) => acc.concat(e), []),
        )
    }
    const children = getChildrenRec(mdle)

    const toRemoveIds = [mdle.moduleId].concat(children.map((m) => m.moduleId))
    const otherModules = project.workflow.modules.filter(
        (m) => !toRemoveIds.includes(m.moduleId),
    )
    const otherPlugins = project.workflow.plugins.filter(
        (m) => !toRemoveIds.includes(m.moduleId),
    )

    const newModules = isPlugin
        ? project.workflow.modules
        : otherModules.concat([newModule])
    let newPlugins = isPlugin ? otherPlugins.concat([newModule]) : otherPlugins
    newPlugins = newPlugins.concat(
        children.map((child) =>
            duplicate({ mdle: child, ready$, parent: newModule }),
        ),
    )
    ...
    */
    ...
    const connections = project.workflow.connections.filter((connection) => {
        if (
            connection.start.moduleId == newModule.moduleId &&
            !remainingOutputSlots.includes(connection.start.slotId)
        ) {
            return false
        }
        if (
            connection.end.moduleId == newModule.moduleId &&
            !remainingInputSlots.includes(connection.end.slotId)
        ) {
            return false
        }
        return true
...
export function alignH(moduleIds: Array<string>, project: Project, ready$) {
    const modulesView = project.builderRendering.modulesView.filter((m) =>
        moduleIds.includes(m.moduleId),
    )
    const modulesViewToKeep = project.builderRendering.modulesView.filter(
        (m) => !moduleIds.includes(m.moduleId),
    )
    const yAverage =
        modulesView.reduce((acc, m) => acc + m.yWorld, 0) / modulesView.length
    const newViews = modulesView.map(
        (m) => new ModuleView(m.moduleId, m.xWorld, yAverage, m.Factory),
    )
    const projectNew = new Project({
        ...project,
        ...{
            builderRendering: new BuilderRendering(
                modulesViewToKeep.concat(newViews),
                project.builderRendering.connectionsView,
                project.builderRendering.descriptionsBoxes,
            ),
        },
...
export function alignV(moduleIds: Array<string>, project: Project, ready$) {
    ...
    const xAverage =
        modulesView.reduce((acc, m) => acc + m.xWorld, 0) / modulesView.length
    const newViews = modulesView.map(
        (m) => new ModuleView(m.moduleId, xAverage, m.yWorld, m.Factory),
    )
    const projectNew = new Project({
        ...project,
        ...{
...
export function duplicateModules(
    ...
    const views = project.builderRendering.modulesView.filter((mView) =>
        modules.map((m) => m.moduleId).includes(mView.moduleId),
    )
    ...
    const newViews = modules.map((m: ModuleFlux, i) => {
        const newView = new ModuleView(
            newModules[i].moduleId,
            maxXWorld + (i + 1) * 50,
            maxYWorld + (i + 1) * 50,
            m.Factory,
        )
    ...
    const parentGroups = modules
        .map((mdle, index) => {
            const parent = project.workflow.modules.find(
                (grp) =>
    ...
    const newGroups = Object.entries(parentGroups).map(
        ([parentId, moduleIds]: [string, string[]]) => {
            const parent = project.workflow.modules.find(
                (mdle) => mdle.moduleId == parentId,
            ) as GroupModules.Module
            const persistentData =
                parent.getPersistentData<GroupModules.Module>()
            const newGroup = updateGroup(parent, {
                moduleIds: persistentData.getModuleIds().concat(...moduleIds),
            })
            return newGroup
        },
    )

    const builderRenderer = new BuilderRendering(
        project.builderRendering.modulesView.concat(newViews),
        project.builderRendering.connectionsView,
        project.builderRendering.descriptionsBoxes,
    )
...
export function moveModules(
    ...
    const explicitModulesPosition = modulesPosition
        .map((modulePosition) => [modulePosition])
        .reduce((acc, e) => acc.concat(e), [])

    const modulesViewToKeep = moduleViews.filter(
        (m) =>
            !explicitModulesPosition
                .map((mNew) => mNew.moduleId)
                .includes(m.moduleId),
    )
    ...
    const modulesViewNew = explicitModulesPosition
        .map((mdle) => [
            mdle,
            moduleViews.find((m) => m.moduleId == mdle.moduleId),
        ])
        .filter(
            ([mdle, mOld]) =>
                Math.abs(mOld.xWorld - mdle.x) > 3 ||
                Math.abs(mOld.yWorld - mdle.y) > 3,
        )
        .map(
            ([mdle, mOld]) =>
                new ModuleView(mdle.moduleId, mdle.x, mdle.y, mOld.Factory),
        )
    ...
    const boxNeedUpdate = project.builderRendering.descriptionsBoxes.find(
        (box) =>
            box.modulesId.length !==
            box.modulesId.filter(
                (mId) =>
                    modulesViewNew.map((m) => m.moduleId).indexOf(mId) >= 0,
            ).length,
    )

    const descriptionBoxes = boxNeedUpdate
        ? project.builderRendering.descriptionsBoxes.map((box) => {
              if (
                  !box.modulesId.find(
                      (mId) =>
                          modulesViewNew.map((m) => m.moduleId).indexOf(mId) >=
                          0,
                  )
              ) {
                  return box
    ...
    const projectNew = new Project({
        ...
        ...{
            builderRendering: new BuilderRendering(
                modulesViewToKeep.concat(modulesViewNew),
                project.builderRendering.connectionsView,
                descriptionBoxes,
            ),
        },
...
export function clonePluginsForNewParents(
    parentModules: ModuleFlux[],
    workflow: Workflow,
) {
    return parentModules
        .map((mdle: ModuleFlux) => {
            return [
                mdle,
                workflow.plugins.find(
                    (p) => p.parentModule.moduleId == mdle.moduleId,
                ),
            ]
        })
...
export function deleteModules(
    modulesDeleted: Array<ModuleFlux>,
    project: Project,
): Project {
    const debugSingleton = AppDebugEnvironment.getInstance()
    ...
    const grpMdlesDeleted = modulesDeleted.filter(
        (mdle) => mdle instanceof GroupModules.Module,
    )
    const inGroupModulesDeleted = grpMdlesDeleted
        .map((grpMdle: GroupModules.Module) =>
            grpMdle.getAllChildren(project.workflow),
        )
        .flat()
    ...
    const parentGroups = project.workflow.modules
        ...
        .filter((mdle: GroupModules.Module) => {
            ...
            const filtered = childIds.filter(
                (mId) => !modulesDeletedId.includes(mId),
            )
    ...
    const pluginsNotReplacedId = project.workflow.plugins
        .filter((plugin) =>
            modulesNotReplacedId.includes(plugin.parentModule.moduleId),
        )
        .map((p) => p.moduleId)
    ...
    const modulesView = project.builderRendering.modulesView.filter(
        (m) => !idsRemoved.includes(m.moduleId),
    )

    const connectionsToKeep = project.workflow.connections.filter(
        (c) =>
            !idsRemoved.includes(c.end.moduleId) &&
    ...
    const boxNeedUpdate = project.builderRendering.descriptionsBoxes.find(
        (box) =>
            ...
            box.modulesId.filter(
                (mId) =>
                    modulesDeleted.map((m) => m.moduleId).indexOf(mId) >= 0,
            ).length,
    )

    const descriptionBoxes = boxNeedUpdate
        ? project.builderRendering.descriptionsBoxes.map((box) => {
              const moduleIdsToKeep = box.modulesId.filter(
                  (mId) =>
                      modulesDeleted.map((m) => m.moduleId).indexOf(mId) < 0,
              )
