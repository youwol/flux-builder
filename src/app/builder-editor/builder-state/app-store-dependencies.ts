...
function updateRequirements(loadingGraph: LoadingGraphSchema, project: Project): Project {
    ...
    const libraries = loadingGraph.lock.reduce((acc, e) => ({ ...acc, ...{ [e.name]: e.version } }), {})
    const packs = loadingGraph.lock
        .filter(library => library.type == 'flux-pack')
        .map(library => library.name )
    const requirements = new Requirements(actualReqs.fluxComponents, Array.from(packs),
...
export function addLibraries(
    ...
    const newPacks = Array.from(new Set(fluxPacks.map(p =>p.name)))
    ...
    .forEach((name) => {
        const install = (window[name].install || window[name].pack.install)(environment)
