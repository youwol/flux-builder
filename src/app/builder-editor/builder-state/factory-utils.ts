...
export function serializeWorkflow(workflow: Workflow): WorkflowSchema {
    return {
        ...
        plugins: workflow.plugins.map((plugin) => ({
            ...
            factoryId: {
                module: plugin.Factory.id,
                pack: plugin.Factory.packId,
            },
            configuration: {
...
export function toProjectData(project: Project) /*ProjectSchema*/ {
    return {
        ...
        builderRendering: {
            descriptionsBoxes: project.builderRendering.descriptionsBoxes.map(
                (d) => ({
            ...
            connectionsView: project.builderRendering.connectionsView.map(
                (c) => ({
