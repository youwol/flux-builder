...
export async function createLayoutEditor(): grapesjs.Editor {
    ...
    if (!bootstrapCss) {
        console.error(
            "Bootstrap css needs to be included in host application with id 'bootstrap-css' ",
        )
    }
    const fontawesomeCss = document.getElementById('fontawesome-css')
    if (!fontawesomeCss) {
        console.error(
            "Fontawesome css needs to be included in host application with id 'fontawesome-css' ",
        )
    }
    const youwolCss = document.getElementById('youwol-css')
    if (!youwolCss) {
        console.error(
            "Youwol css needs to be included in host application with id 'youwol-css' ",
        )
    }
    ...
    return new Promise((successCb) =>
        editor$.pipe(take(1)).subscribe((edtr) => successCb(edtr)),
    )
}
