...
import { c, logFactory, v } from '../logging'

const log = logFactory().getChildLogger('Navigation')
...
export function navigate(
    ...
    function getComponents(): Component.Module[] {
        ...
        return workflow.modules
            .filter(
                (maybeComponent) => maybeComponent instanceof Component.Module,
            )
    ...
    return {
        ...
        fromGroup(moduleId: string): ModuleNavigation<GroupModules.Module> {
            _log.debug('fromGroup {0}', v(moduleId))
        ...
        fromComponent(moduleId: string): ModuleNavigation<Component.Module> {
            _log.debug('fromComponent {0}', v(moduleId))
            const matching = this.fromComponentMatching(
                predicateModuleId(moduleId),
            )
            if (!matching) {
...
function factoryModuleNavigation<T extends ModuleFlux>(
    ...
    return {
        ...
        toDescendantsMatching(
            ...
            this.toChildren().forEach(
                (childNav: ModuleNavigation<ModuleFlux>) => {
                    descendants.push(childNav)
                    if (traversable(childNav.get())) {
                        descendants.push(
                            ...childNav.toDescendantsMatching(
                                predicate,
                                traversable,
                            ),
                        )
        ...
        parentModule = navigate(workflow)
            .fromGroupMatching(
                (group) =>
                    group
                        .getModuleIds()
                        .find((childId) => childId === mdle.moduleId) !==
                    undefined,
            )
            .get()
    }
