...
import {Component, GroupModules, ModuleFlux, PluginFlux, WorkflowDelta} from "@youwol/flux-core";
import {
    getMockedAppStore,
    getMockedComponent,
...
describe('ProjectTreeView.nodeIdBuilder', () => {
    ...
    it('should return node id for module containing both moduleId and unique string', () => {
        ...
        // When
        const actualNodeId = subject.buildForModule(getMockedModule({moduleId: moduleId}))
    ...
    it('should return node id for root component containing both ' +
    ...
    it('should not be instantiable if unique string is undefined, null or empty', () => {
        ...
        const uniqNull = null
        const uniqEmpty = ""
        const expectedError = new Error("Unique string must be defined and not empty")
...
describe('ProjectTreeView.rootFactory', () => {
    ...
    it('should failed if project has no root module', () => {
        ...
        const mockModule = getMockedRootModule({moduleId: "NOT_ROOT_COMPONENT_ID"})
        const nodeIdBuilder = getMockedNodeIdBuilder([{mdle: mockModule, nodeId: nodeId}])
    ...
    it('should return tree of nodes with only root node for empty project', () => {
        ...
        const mockNodeIdBuilder = getMockedNodeIdBuilder([{mdle: mockModule, nodeId: expectedNodeId}])
        const mockWorkflow = getMockedWorkflow({modules: [mockModule]})
    ...
    it('should failed for module of unknown class', () => {
        ...
        const expectedError = new Error("Unknown module class for project tree view node creation")

        const mockRootModule = getMockedRootModule({children: [mockUnknownModule]})
        const mockWorkflow = getMockedWorkflow({modules: [mockUnknownModule, mockRootModule]})
        const mockNodeIdBuilder = getMockedNodeIdBuilder()


        // Expect
        expect(() => subject(mockWorkflow, mockNodeIdBuilder)).toThrow(expectedError)
    })
    ...
    it('should return a tree of nodes for a simple workflow', () => {
        ...
        const mockPlugin: PluginFlux<any> = getMockedPlugin({
...
describe('ProjectTreeView.nodeHeaderView', () => {
    ...
    it('should return icon fa-play and project name for root node', () => {
        ...
        const mockState = getMockedState({appStore: {project: {projectName: expectedProjectName}}})

        // When
        document.body.appendChild(render(subject(mockState, mockRootNode)))
        const actualIcon = document.querySelector(`.${expectedClass}`)
        const actualDivText = document.querySelector('.project-tree-node').children[1] as HTMLDivElement
    ...
    it('should return module title for ModuleNode', () => {
        ...
        const mockModuleNode = getMockedModuleNode({moduleTitle: expectedModuleTitle})
    ...
    it('should return icon fa-object-group and module title for GroupNode', () => {
        ...
        const mockGroupNode = getMockedGroupNode({moduleTitle: expectedGroupTitle})
        const mockState = getMockedState()
    ...
    it('should return icon fa-cube and module title for ComponentNode', () => {
        ...
        const mockComponentNode = getMockedComponentNode({moduleTitle: expectedComponentTitle})
        const mockState = getMockedState()
    ...
    it('should return icon fa-puzzel-piece and module title for PluginNode', () => {
        ...
        const mockPluginNode = getMockedPluginNode({moduleTitle: expectedPluginTitle})
        const mockState = getMockedState()
...
describe('ProjectTreeView.State', () => {
    ...
    nodeIdBuilder = ProjectTreeView.nodeIdBuilderForUniq(uniq)
    ...
    const expectedPluginId = nodeIdBuilder.buildForModuleId(expectedPluginModuleId)
    ...
    beforeEach(() => {
        ...
        subject = ProjectTreeView.State.stateForAppStoreAndUniq(getMockedAppStore({
                project: {
                    workflow: {
                        modules: [mockRootModule, mockModule, mockGroup, mockPlugin],
                        plugins: [mockPlugin]
    ...
    it('should rebuild tree in response to appStore project update', () => {
        ...
        // When
        const actualOldGroupTitle = subject.getNode(expectedGroupId).getModuleTitle()
        ...
        const actualNewGroupTitle = subject.getNode(expectedGroupId).getModuleTitle()
    ...
    it('should unsubscribe its own subscriptions ', () => {
        ...
        // When
        subject.unsubscribe()
        const actualDeltaObserversProjectUpdate = observersProjectUpdatedBefore - appStoreProjectUpdated$.observers.length
        const actualDeltaObserversSelection = observersSelectionBefore - appStoreSelection$.observers.length

        // Expect
        expect(actualDeltaObserversProjectUpdate).toBe(expectedDeltaObserversProjectUpdate)
