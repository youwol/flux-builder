...
test('group modules module creation & BuilderView', (done) => {
    ...
    appObservables.projectUpdated$.pipe(take(1)).subscribe(() => {
        appStore.addGroup(['module1'])
    })
    appObservables.projectUpdated$.pipe(take(1)).subscribe((delta) => {
        /*
                          |~module2~|\
                                      \
                                       \
        |~module3~|-----||~module1~||----|~module0~|-
                        ||~module1~||----|~plugin ~|
        */
        const groupModule = delta.modules.createdElements.find(
            (mdle) => mdle.moduleId != appStore.rootComponentId,
        ) as GroupModules.Module

        const connections = groupModule.getConnections(
    ...
    appObservables.projectUpdated$.pipe(take(1)).subscribe((delta) => {
        ...
        |~module3~|-----||~module1~||----|~module0~|-
                        ...
                        ||~module1~||----|~module2 ~|
        */
        const groupModule = appStore.project.workflow.modules.find((mdle) =>
            mdle.moduleId.includes('GroupModules_'),
        ) as GroupModules.Module
