...
test('test info debugs', () => {
    ...
    debugSingleton.logAppTopic({
        level: LogLevel.Info,
        message: 'test',
        object: { index: 0 },
    })
    debugSingleton.logObservable({
        level: LogLevel.Info,
        message: 'test',
        object: { index: 1 },
    })
    ...
    debugSingleton.logWorkflowView({
        level: LogLevel.Info,
        message: 'test',
        object: { index: 4 },
    })
    ...
    const targets = [
        {
            message: '#App',
            object: { level: 1, message: 'test', object: { index: 0 } },
        },
        {
            message: '#Observables',
            object: { level: 1, message: 'test', object: { index: 1 } },
        },
        {
            message: '#Render',
            object: { level: 1, message: 'test', object: { index: 2 } },
        },
        {
            message: '#WorkflowBuilder',
            object: { level: 1, message: 'test', object: { index: 3 } },
        },
        {
            message: '#WorkflowView',
            object: { level: 1, message: 'test', object: { index: 4 } },
        },
    ]
    targets.forEach((target, i) => {
        ...
        expect(target.object.object.index).toEqual(
            messages[i].object.object.index,
        )
    })
})
