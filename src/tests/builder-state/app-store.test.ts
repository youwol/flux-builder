...
test('load empty project', (done) => {
    ...
    appObservables.ready$.pipe(filter((d) => d)).subscribe(() => {
        expect(appStore.project.workflow.modules).toHaveLength(1)
        expect(appStore.project.workflow.modules[0]).toBeInstanceOf(
            Component.Module,
        )
...
test('load simple project', (done) => {
    ...
    appObservables.ready$.pipe(filter((d) => d)).subscribe((d) => {
        expect(appStore.project.workflow.modules).toHaveLength(
            projectData.workflow.modules.length,
        )
        expect(appStore.project.workflow.connections).toHaveLength(
            projectData.workflow.connections.length,
        )
        expect(appStore.project.workflow.plugins).toHaveLength(
            projectData.workflow.plugins.length,
        )

        expect(
            appStore.project.workflow.modules[0].configuration.title,
        ).toEqual(projectData.workflow.modules[0].configuration.title)

        expect(
            appStore.project.workflow.modules[0].configuration.data.property0,
...
test('save project 0', (done) => {
    ...
    appObservables.modulesUpdated$.subscribe(() => {
        ...
        expect(saved.builderRendering).toEqual(
            projects.simpleProject.builderRendering,
        )
...
test('save project 1', (done) => {
    ...
    appObservables.modulesUpdated$.subscribe(() => {
        ...
        expect(saved.requirements).toEqual(
            projects.simpleProjectConnection.requirements,
        )
        expect(saved.builderRendering).toEqual(
            projects.simpleProjectConnection.builderRendering,
        )
        expect(saved.workflow).toEqual(
            projects.simpleProjectConnection.workflow,
        )
        done()
    })
...
test('selection', (done) => {
    ...
    appObservables.ready$.pipe(filter((d) => d)).subscribe((d) => {
        ...
        expect(appStore.descriptionBoxSelected.descriptionBoxId).toBe(
            'descriptionBoxId',
        )
