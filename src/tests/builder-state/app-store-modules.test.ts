...
test('update module', () => {
    ...
    appStore.updateModule(
        mdle,
        new SimpleModule['Configuration']({
            title: 'new title',
            description: '',
            data: { property0: 1 },
        }),
    ...
    expect(appStore.getRootComponent().getModuleIds()).toHaveLength(1)
    const newMdle = appStore.project.workflow.modules.find(
        (mdle) => mdle instanceof SimpleModule.Module,
    )
    ...
    appStore.undo()
    const prevMdle = appStore.project.workflow.modules.find(
        (mdle) => mdle instanceof SimpleModule.Module,
    )
    expect(prevMdle).toEqual(mdle)
    appStore.redo()
    const newMdleRe = appStore.project.workflow.modules.find(
        (mdle) => mdle instanceof SimpleModule.Module,
    )
...
test('move module', () => {
    ...
    function getView(id) {
        return appStore.project.builderRendering.modulesView.find(
            (m) => m.moduleId == id,
        )
    }
    ...
    const mdle2 = appStore.project.workflow.modules.find(
        (m) => m instanceof SimpleModule.Module && m != mdle,
    )
    ...
    appStore.moveModules([
        { moduleId: activeModules[0].moduleId, translation: [10, 0] },
    ])
