...
test('add 2 module and connections + description box ', () => {
    ...
    const descriptionBox = new DescriptionBox(
        'descriptionBoxId',
        'title',
        [mdle0.moduleId, mdle1.moduleId],
        '',
        properties,
    )
    appStore.addDescriptionBox(descriptionBox)

    expect(appStore.project.builderRendering.descriptionsBoxes).toHaveLength(1)
    expect(appStore.project.builderRendering.descriptionsBoxes[0]).toEqual(
        descriptionBox,
    )

    appStore.undo()
    expect(appStore.project.builderRendering.descriptionsBoxes).toHaveLength(0)
    appStore.redo()
    expect(appStore.project.builderRendering.descriptionsBoxes).toHaveLength(1)
    expect(appStore.project.builderRendering.descriptionsBoxes[0]).toEqual(
        descriptionBox,
...
test('add 2 module and connections + description box + update', () => {
    ...
    const descriptionBoxNew = new DescriptionBox(
        'descriptionBoxId',
        'title2',
        [mdle0.moduleId],
        '',
