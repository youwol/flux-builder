...
function setupProject({ modulesCount }: { modulesCount: number }): any {
    ...
    const mdles = workflow.modules.filter(
        (mdle) => mdle instanceof SimpleModule.Module,
    ) as SimpleModule.Module[]
...
test('instantiate connections', () => {
    ...
    const connectionsData = [
        {
            start: {
                moduleId: mdle0.moduleId,
                slotId: mdle0.outputSlots[0].slotId,
            },
            end: {
                moduleId: mdle1.moduleId,
                slotId: mdle1.inputSlots[0].slotId,
            },
        },
        {
            start: {
            ...
            adaptor: {
                adaptorId: 'adaptor0',
                mappingFunction: 'return (input) => input',
            },
        },
        {
    ...
    const connections = instantiateProjectConnections(
        appStore.allSubscriptions,
        connectionsData,
        appStore.project.workflow.modules,
    )
...
test('create connections + module deletion', () => {
    ...
    const [appStore, mdle0, mdle1, mdle2, mdle3] = setupProject({
        modulesCount: 4,
    })

    appStore.addConnection(
        new Connection(mdle1.outputSlots[0], mdle0.inputSlots[0]),
    )
    appStore.addConnection(
        new Connection(mdle2.outputSlots[0], mdle1.inputSlots[0]),
    )
    appStore.addConnection(
        new Connection(mdle3.outputSlots[0], mdle2.inputSlots[0]),
    )
...
test('create connections + module update', () => {
    ...
    appStore.updateModule(
        mdle1,
        new SimpleModule['Configuration']('new title', '', { property0: 1 }),
    )
