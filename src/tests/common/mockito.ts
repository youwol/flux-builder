...
import {AppObservables, AppStore} from "../../app/builder-editor/builder-state";
...
export function getMockedNodeIdBuilder(nodesIdsForModules: { mdle: ModuleFlux, nodeId: string }[] = [],
                                       nodesIdsForModulesIds: { moduleId: string, nodeId: string }[] = []) {

    const nodeIdBuilderMocker = mock<ProjectTreeView.NodeIdBuilder>()

    when(nodeIdBuilderMocker.buildForRootComponent()).thenReturn(Component.rootComponentId)

    nodesIdsForModules.forEach((entry) =>
        when(nodeIdBuilderMocker.buildForModule(entry.mdle)).thenReturn(entry.nodeId)
    )

    nodesIdsForModulesIds.forEach((entry) =>
        when(nodeIdBuilderMocker.buildForModuleId(entry.moduleId)).thenReturn(entry.nodeId)
    )
...
export function getMockedModuleConfiguration(fixture?: FixtureModuleConfiguration | ModuleConfiguration): ModuleConfiguration {
    if (fixture instanceof ModuleConfiguration) {
        return fixture
    }
    const finalFixture: FixtureModuleConfiguration = {...defaultsFlux.moduleFlux.fixture.moduleConfiguration, ...fixture}
    ...
    Object.setPrototypeOf(moduleConfigurationMocker, ModuleConfiguration.prototype)
...
export function getMockedModule(fixture?: FixtureModuleFlux | ModuleFlux): ModuleFlux {
    if (fixture instanceof ModuleFlux) {
        return fixture
    }
    const finalFixture: FixtureModuleFlux = {...defaultsFlux.moduleFlux.fixture, ...fixture}
    const moduleFluxMocker = mock(ModuleFlux)
    when(moduleFluxMocker.moduleId).thenCall(convert(finalFixture.moduleId))
    when(moduleFluxMocker.configuration).thenReturn(getMockedModuleConfiguration(finalFixture.moduleConfiguration))
...
export function getMockedGroup(fixture?: FixtureGroupModule) {
    const finalFixture: FixtureGroupModule = {...defaultsFlux.groupFlux.fixture, ...fixture}
    const groupMocker = mock(GroupModules.Module)
    when(groupMocker.moduleId).thenCall(convert(finalFixture.moduleId))
    when(groupMocker.configuration).thenReturn(getMockedModuleConfiguration(finalFixture.moduleConfiguration))
    when(groupMocker.getDirectChildren(anything())).thenReturn(finalFixture.children)
...
export function getMockedComponent(fixture?: FixtureComponentModule) {
    const finalFixture: FixtureComponentModule = {...defaultsFlux.componentFlux.fixture, ...fixture}
    const componentMocker = mock(Component.Module)
    when(componentMocker.moduleId).thenCall(convert(finalFixture.moduleId))
    when(componentMocker.configuration).thenReturn(getMockedModuleConfiguration(finalFixture.moduleConfiguration))
    when(componentMocker.getDirectChildren(anything())).thenReturn(finalFixture.children)
...
export function getMockedPlugin(fixture: FixturePlugin) {
    const finalFixture: FixturePlugin = {...defaultsFlux.pluginFlux.fixture, ...fixture}

    const pluginFluxMocker = mock(PluginFlux)
    when(pluginFluxMocker.moduleId).thenCall(convert(finalFixture.moduleId))
    when(pluginFluxMocker.configuration).thenReturn(getMockedModuleConfiguration(finalFixture.moduleConfiguration))
...
interface FixtureRootModule extends FixtureGroupModule {
}

export function getMockedRootModule(fixture?: FixtureRootModule): Component.Module {
    const finalFixture: FixtureRootModule = {...defaultsFlux.rootFlux.fixture, ...fixture}
...
interface FixtureWorkflow {
    modules?: ModuleFlux[]
    plugins?: PluginFlux<any>[]
}

export function getMockedWorkflow(fixture?: FixtureWorkflow | Workflow): Workflow {
    if (fixture instanceof Workflow) {
        return fixture
    }
    const finalFixture: FixtureWorkflow = {...defaultsFlux.workflow.fixture, ...fixture}
...
export function getMockedProject(fixture?: FixtureProject | Project): Project {
    ...
    const finalFixture: FixtureProject = {...defaultsFlux.project.fixture, ...fixture}
    const projectMocker = mock(Project)
    when(projectMocker.workflow).thenReturn(getMockedWorkflow(finalFixture.workflow))
...
export function getMockedAppObservables(fixture?: FixtureAppObservables | AppObservables): AppObservables {
    if (fixture instanceof AppObservables) {
        return fixture
    }
    const finalFixture: FixtureAppObservables = {...defaultAppStore.appObservables, ...fixture}
    const appObservablesMocker = mock(AppObservables)

    when(appObservablesMocker.projectUpdated$).thenReturn(finalFixture.projectUpdated)

    when(appObservablesMocker.moduleSelected$).thenReturn(finalFixture.moduleSelected)
...
export function getMockedAppStore(fixture?: FixtureAppStore | AppStore, customizeMocker: (mocker) => void = (m) => {}): AppStore {
    if(fixture instanceof  AppStore) {
        return fixture
    }
    const finalFixture: FixtureAppStore = {...defaultAppStore.appStore, ...fixture}
    const appStoreMocker = mock(AppStore)
    when(appStoreMocker.project).thenReturn(getMockedProject(finalFixture.project))
    when(appStoreMocker.appObservables).thenReturn(getMockedAppObservables(finalFixture.appObservables))
...
// CustomizeMocker should be generalized and better exposed to caller
export function getMockedState(fixture?: FixtureState, customizeMocker: (mocker) => void = (m) => {}) {
    const finalFixture: FixtureState = {...defaultFixtureState, ...fixture}
    const stateMocker = mock(ProjectTreeView.State)
    when(stateMocker.appStore).thenReturn(getMockedAppStore(finalFixture.appStore))
...
export function getMockedModuleNode(fixture?: FixtureModuleNode) {
    const finalFixture: FixtureModuleNode = {...defaultFixtureModuleNode, ...fixture}
...
interface FixtureGroupNode extends FixtureModuleNode {
}
...
export function getMockedGroupNode(fixture?: FixtureGroupNode) {
    const finalFixture: FixtureGroupNode = {...defaultFixtureGroupNode, ...fixture}
...
interface FixtureComponentNode extends FixtureGroupNode {
}
...
export function getMockedComponentNode(fixture?: FixtureComponentNode) {
    const finalFixture: FixtureComponentNode = {...defaultFixtureComponentNode, ...fixture}
...
interface FixturePluginNode extends FixtureModuleNode {
}
...
export function getMockedPluginNode(fixture?: FixturePluginNode) {
    const finalFixture: FixturePluginNode = {...defaultFixturePluginNode, ...fixture}
