...
import { NodeIdBuilder } from '../../app/externals_evolutions/fv-tree/immutable-tree'
...
export function getMockedNodeIdBuilder(
    ...
    const nodeIdBuilderMocker = mock<NodeIdBuilder>()
...
export function getMockedProjectManager(
    ...
    when(projectManagerMocked.filterSelection(anything())).thenCall(
        (moduleId) => finalFixture.filterSelection(moduleId),
    )
