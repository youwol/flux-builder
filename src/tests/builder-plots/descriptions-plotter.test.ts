...
import {
    ModulesPlotter,
    DescriptionsBoxesPlotter,
} from '../../app/builder-editor/builder-plots/index'
...
test('load simple project + test its unique description box rendering', (done) => {
    ...
    const mdlesPlotter = new ModulesPlotter(
        drawingArea,
        plottersObservables,
        appObservables,
        appStore,
    )
    const descriptionsPlotter = new DescriptionsBoxesPlotter(
        drawingArea,
        plottersObservables,
        appObservables,
        appStore,
    ...
    const descriptionBoxSaved =
        projects.simpleProject.builderRendering.descriptionsBoxes[0]
    plottersObservables.descriptionsBoxesDrawn$
        .pipe(skip(0), take(1))
        .subscribe((svgElements) => {
            expect(svgElements).toHaveLength(1)
            expect(svgElements[0].id).toEqual(
                descriptionBoxSaved.descriptionBoxId,
            )
    ...
    plottersObservables.descriptionsBoxesDrawn$
        .pipe(skip(1), take(1))
        .subscribe((d) => {
            const descriptionBox = document.getElementById(
                descriptionBoxSaved.descriptionBoxId,
            )
