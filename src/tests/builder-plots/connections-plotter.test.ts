...
test('load simple project and test connections plot', (done) => {
    ...
    const connectionsPlotter = new ConnectionsPlotter(
        drawingArea,
        plottersObservables,
        appObservables,
        appStore,
    ...
    plottersObservables.connectionsDrawn$.pipe(take(1)).subscribe(() => {
        ...
        expect(
            slotsId.includes('input-slot_input0-plugin_plugin0'),
        ).toBeTruthy()
    ...
    plottersObservables.plugOutputClicked$.pipe(take(1)).subscribe(() => {
        ...
        const newConnection = document.getElementById(
            'output0@module3-input0@module0',
        )
        const event = new MouseEvent('click', { button: 0 })
