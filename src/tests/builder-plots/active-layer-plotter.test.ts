...
test('load simple project', (done) => {
    ...
    appObservables.ready$.pipe(filter((d) => d)).subscribe((_) => {
    ...
    plottersObservables.modulesDrawn$
        ...
        .subscribe((svgElements) => {
            ...
            const elemInDocument = document.getElementById(
                'expanded_GroupModules_child-layer',
            )

            expect(elemInDocument).toBeDefined()
            const x = elemInDocument.getAttribute('x'),
                y = elemInDocument.getAttribute('y')
            expect(x).toBe('60')
            expect(y).toBe('50')
    ...
    plottersObservables.modulesDrawn$.pipe(skip(1), take(1)).subscribe((_) => {
        const layerSvg = document.querySelector('.active-layer-box')
        expect(layerSvg).toBeFalsy()
        appStore.selectActiveGroup('GroupModules_child-layer')
    })
    plottersObservables.modulesDrawn$.pipe(skip(2), take(1)).subscribe((_) => {
