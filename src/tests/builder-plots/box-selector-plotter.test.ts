...
test('load simple project', (done) => {
    ...
    plottersObservables.modulesDrawn$.pipe(skip(0), take(1)).subscribe((d) => {
        const selectorPlotter = new BoxSelectorPlotter(
            drawingArea,
            plottersObservables,
            appObservables,
            appStore,
        ...
        const rectSelectorSvg = document.querySelectorAll(
